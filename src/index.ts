...
async function connect() {
    ...
    const selectedAddresses = await web3.eth.getAccounts();
...
async function signMessage() {
    ...
    const selectedAddresses = await web3.eth.getAccounts();
    const selectedAddress = selectedAddresses[0];
    await web3.eth.personal.sign(text, selectedAddress, '');
}
...
window.onload = function() {
    ...
    document.getElementById("sign-two-txs")!.addEventListener("click", (event) => {
        event.preventDefault();
        sendTx().then(() => {
            sendTx().then(console.log).catch(console.error);
        }).catch(console.error);
    });
    document.getElementById("sign-message-tx")!.addEventListener("click", (event) => {
        event.preventDefault();
        signMessage().then(() => {
            sendTx().then(console.log).catch(console.error);
        }).catch(console.error);
